...
- The `history` daemon automatically swaps to an updated installed executable on the next action, resetting focus history if the binary bytes changed. To swap immediately after building and installing, invoke `herdr plugin action invoke gjermundgaraba.herdr-history.activate`. Installing identical bytes does not restart it.
