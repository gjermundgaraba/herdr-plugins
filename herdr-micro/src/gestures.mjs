...
export class GestureDispatcher {
  ...
  handle(key, binding, pressed) {
    if (!pressed) {
      this.#release(key);
      return;
    }
    if (!isGestureBinding(binding)) {
      if (binding) this.fire(binding, key);
      return;
    }
    this.#press(key, binding);
  }
  ...
  #press(key, binding) {
    ...
    if (binding.hold) {
      state.holdTimer = this.schedule(() => {
        ...
        state.held = true;
        state.doubleTap = null;
        this.fire(binding.hold, `${key} hold`);
      }, binding.holdMs ?? 500);
    }
  ...
  #release(key) {
    ...
    if (binding.release) this.fire(binding.release, `${key} release`);
    if (state.held) {
      this.#states.delete(key);
    } else if (state.doubleTap) {
      this.fire(state.doubleTap, `${key} double-tap`);
      this.#states.delete(key);
    } else if (binding.doubleTap) {
      state.pending = {
        binding,
        timer: this.schedule(() => {
          if (binding.tap) this.fire(binding.tap, `${key} tap`);
          this.#states.delete(key);
